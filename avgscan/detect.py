...
def _find_naw(text, loc, out):
    for m in _POSTCODE_RE.finditer(text):
        window = text[m.start():min(len(text), m.end() + 25)]
        # Postcode + nabij huisnummer => NAW-signaal.
        if _HUISNR_NABIJ_RE.search(window[len(m.group(0)):]):
...
def annotate_naw_context(findings: list[Finding], chunks) -> list[Finding]:
    ...
    if not chunks:
        return findings
    subject = {_norm_pc(m.group(0)) for m in _POSTCODE_RE.finditer(chunks[0][1] or "")}
    if not subject:
        return findings
    for f in findings:
